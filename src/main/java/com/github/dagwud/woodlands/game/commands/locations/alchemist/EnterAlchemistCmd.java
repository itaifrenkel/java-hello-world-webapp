...
public class EnterAlchemistCmd extends AbstractCmd
{
  ...
  public void execute()
  {
    ...
    boolean playerIsNotEnchanting = character.produceItems(ConsumableTrinket.class).isEmpty();
    ...
    {
      giveEntryAllowedText(playerIsNotEnchanting);
    }

    if (playerIsNotEnchanting || character.getParty().getAlchemist().isBusyCrafting())
    {
      CommandDelegate.execute(new MoveToLocationCmd(character, ELocation.VILLAGE_SQUARE));
    }
  }

  private void giveEntryAllowedText(boolean playerHasNoEnchantingItems)
  {
    if (playerHasNoEnchantingItems)
    {
      CommandDelegate.execute(new SendMessageCmd(character, "The alchemist can't work with your smelly socks alone - come back when you have something to enchant with. Be more enchanting."));
    }
  ...
  private void handleCollection(EquippableItem collected)
  {
    String msg;
    if (character.getParty().getAlchemist().isBusyCrafting())
    {
